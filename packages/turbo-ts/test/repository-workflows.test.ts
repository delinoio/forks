...
describe("repository workflow gate", () => {
  ...
  it(evidenceId.repositoryWorkflows, async () => {
    ...
    expect(
      isInternalRepositoryPath(
        nestedRepository,
        join(nestedRepository, ".venv", "lib", "generated.py"),
      ),
    ).toBe(true);
    expect(
      isInternalRepositoryPath(
    ...
    expect(
      parseDaemonArguments(["start", "--idle-time=2147483647ms"])
        .idleMilliseconds,
    ).toBe(2_147_483_647);
    for (const idleTime of ["30d", "2147483648ms", `${"9".repeat(400)}ms`]) {
      expect(() =>
        parseDaemonArguments(["start", `--idle-time=${idleTime}`]),
      ).toThrow(`invalid idle time: ${idleTime}`);
    }
    expect(
  ...
  it("rejects structured-log output collisions case-insensitively on Windows", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-log-output-case-"),
    );
    const logPath = join(directory, "packages/app/Build/run.ndjson");
    try {
      await prepareFixture(directory);
      await expect(
        Effect.runPromise(
      ...
      expect(
        await readFile(logPath, "utf8").catch(() => undefined),
      ).toBeUndefined();
    } finally {
      await rm(directory, { force: true, recursive: true });
    }
  ...
  it("does not treat a root-scoped Cargo package as a child workspace", async () => {
    const directory = await mkdtemp(join(tmpdir(), "turbo-ts-root-cargo-"));
    try {
      await writeFile(
      ...
      expect(
        (
          JSON.parse(completedRun.stdout.trim().split("\n").at(-1)!) as {
            readonly monorepo: boolean;
          }
        ).monorepo,
      ).toBe(false);
  ...
  it("decodes package and changed-output daemon responses", async () => {
    if (process.platform === "win32") return;
    const directory = await mkdtemp(join(tmpdir(), "turbo-ts-daemon-client-"));
    const socket = join(directory, "turbod.sock");
    try {
      const responses = await Effect.runPromise(
        Effect.scoped(
          Effect.gen(function* () {
            ...
            yield* Effect.promise(() => waitUntil(() => existsSync(socket)));
            const discovered = yield* daemon.request(socket, {
              id: "discover",
              method: DaemonMethod.discoverPackages,
            });
            const changed = yield* daemon.request(socket, {
              id: "changed",
      ...
      expect(responses.discovered.result).toEqual({
        packages: [{ name: "synthetic-app", path: "packages/app" }],
        packageManager: "pnpm9",
      });
      expect(responses.changed.result).toEqual({
        changedOutputs: [
          {
            hash: "synthetic-hash",
            changedOutputGlobs: ["dist/*.js"],
                  ...
                  watch: (root) =>
                    Stream.fromEffect(Deferred.await(registered)).pipe(
                      Stream.flatMap(() =>
                        Stream.fromIterable([
                          {
                            path: join(root, ".turbo/generated/output.js"),
                          ...
                          },
                          {
                            path: join(
                              root,
                              "node_modules/generated/output.js",
                            ),
                            kind: "modify" as const,
                            entryKind: "file" as const,
                          },
                          ...
                          {
                            path: join(root, ".turbo/daemon/synthetic.log"),
                            kind: "modify" as const,
                            entryKind: "file" as const,
                          },
                        ]),
  ...
  it("propagates daemon state-directory cleanup failures", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-daemon-clean-failure-"),
    );
    let stateDirectory: string | undefined;
    try {
      await prepareFixture(directory);
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const failure = new BoundaryError({
            boundary: "filesystem",
            message: "synthetic state cleanup failure",
            retryable: true,
          });
          return yield* executeDaemon({
            command: "clean",
  ...
  it("preserves daemon state while status and logs wait for startup", async () => {
    const directory = await mkdtemp(join(tmpdir(), "turbo-ts-daemon-ready-"));
    try {
      await prepareFixture(directory);
      await Effect.runPromise(
        Effect.scoped(
          Effect.gen(function* () {
            ...
            const logsFiber = yield* Effect.forkScoped(
              runDaemon("logs").pipe(
                Effect.tap(() =>
                  Effect.sync(() => {
                    logsCompleted = true;
            ...
            expect(logsCompleted).toBe(false);
            ...
            yield* Deferred.succeed(allowServe, undefined);
            expect(yield* Fiber.join(statusFiber)).toBe(0);
            for (
              let attempt = 0;
              attempt < 100 && !stdout.includes("daemon started");
              attempt += 1
            ) {
            ...
            expect(logsCompleted).toBe(false);
            yield* Fiber.interrupt(logsFiber);
  ...
  it("refreshes an ignored active workspace configuration", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-watch-ignored-config-"),
    );
    try {
      await prepareFixture(directory);
      const configurationPath = join(directory, "packages/app/turbo.json");
      ...
      const configurationChanges = Stream.fromEffect(
        Effect.sleep("250 millis").pipe(
          Effect.zipRight(
            Effect.promise(async () => {
              const configuration = JSON.parse(
      ...
      await Effect.runPromise(
        executeWatch(
          ...
          Effect.provide(
            Layer.succeed(FileWatcherService, {
              watch: (root) =>
                root === directory ? configurationChanges : Stream.empty,
            }),
          ),
          Effect.provide(nodeFoundationLayer),
        ),
  ...
  it("uses single-package discovery when filtering watch changes", async () => {
    ...
    const closed = new Promise<void>((resolve) => child.once("close", resolve));
    try {
      await waitUntil(() => stdout.includes("root build"));
      await new Promise((resolve) => setTimeout(resolve, 500));
      const initialRuns = (stdout.match(/root build/g) ?? []).length;
      await writeFile(
        join(directory, "packages/app/build/source.txt"),
        "root input\n",
      );
      await waitUntil(
        () => (stdout.match(/root build/g) ?? []).length > initialRuns,
      );
      expect(stdout).not.toContain("app build");
    } finally {
      child.kill();
      await Promise.race([
        closed,
  ...
  it("reloads ignored control files before watch output filtering", async () => {
    const directory = await mkdtemp(join(tmpdir(), "turbo-ts-watch-ignore-"));
    await prepareFixture(directory);
    await writeFile(join(directory, ".gitignore"), ".gitignore\nignored.txt\n");
    const nestedIgnorePath = join(directory, "packages/app/build/.gitignore");
    await mkdir(dirname(nestedIgnorePath), { recursive: true });
    await writeFile(nestedIgnorePath, ".gitignore\n");
    const child = spawn(
      process.execPath,
      [
        candidate,
    ...
    try {
      await waitUntil(() => stdout.includes("app build"));
      await new Promise((resolve) => setTimeout(resolve, 500));
      let runs = (stdout.match(/app build/g) ?? []).length;
      await writeFile(join(directory, ".gitignore"), ".gitignore\n");
      await waitUntil(() => (stdout.match(/app build/g) ?? []).length > runs);
      await new Promise((resolve) => setTimeout(resolve, 500));
      runs = (stdout.match(/app build/g) ?? []).length;
      await writeFile(join(directory, "ignored.txt"), "now visible\n");
      await waitUntil(() => (stdout.match(/app build/g) ?? []).length > runs);
      await new Promise((resolve) => setTimeout(resolve, 500));
      runs = (stdout.match(/app build/g) ?? []).length;
      await writeFile(nestedIgnorePath, ".gitignore\nchanged.txt\n");
  ...
  it("suppresses artifacts written by watched runs", async () => {
    const directory = await mkdtemp(
    ...
    const child = spawn(
      ...
      [
        ...
        "--experimental-write-cache",
        "--cache=local:rw",
        "--cache-dir=packages/app/custom-cache",
        "--log-file=packages/app/run.ndjson",
        "--profile=packages/app/profile.json",
        "--trace=packages/app/trace.json",
        "--heap=packages/app/run.heapsnapshot",
        "--cwd",
        directory,
      ],
      ...
      await waitUntil(
        () =>
          ...
          [
            ...
            "trace.json",
            "run.heapsnapshot",
          ].every((path) => existsSync(join(directory, "packages/app", path))),
        30_000,
      );
      const appBuildOutputCount = (stdout.match(/app build/g) ?? []).length;
      await new Promise((resolve) => setTimeout(resolve, 1_000));
  ...
  it("refreshes tracked ignored inputs after the Git index changes", async () => {
    ...
    configuration.tasks = {
      tracked: { cache: false, inputs: ["tracked.txt"] },
    };
    await writeFile(
      configurationPath,
      `${JSON.stringify(configuration, undefined, 2)}\n`,
    ...
    applicationManifest.scripts.tracked =
      "node -e \"console.log('tracked input run')\"";
    await writeFile(
      applicationManifestPath,
      `${JSON.stringify(applicationManifest, undefined, 2)}\n`,
    );
    const trackedInput = join(applicationDirectory, "tracked.txt");
  ...
  it("treats metadata-free output removals as directory events", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-watch-output-removal-"),
    );
    try {
      await prepareFixture(directory);
      const applicationDirectory = join(directory, "packages/app");
      const configurationPath = join(applicationDirectory, "turbo.json");
      const configuration = JSON.parse(
        await readFile(configurationPath, "utf8"),
      ) as { tasks: { build: { outputs: Array<string> } } };
      ...
      manifest.scripts.build =
        "node -e \"const fs=require('node:fs');fs.mkdirSync('dist',{recursive:true});fs.rmSync('dist',{recursive:true});fs.appendFileSync('.watch-runs','run\\n')\"";
      await writeFile(
        manifestPath,
        `${JSON.stringify(manifest, undefined, 2)}\n`,
      );
                ...
                root === directory
                  ? Stream.fromEffect(
                      Effect.promise(async () => {
                        await waitUntil(() => existsSync(runsPath));
                        return {
                          path: join(applicationDirectory, "dist"),
                          kind: "remove" as const,
                        };
                      }),
                    )
  ...
  it("bounds and deduplicates GraphQL repository file reads", async () => {
    const directory = await mkdtemp(join(tmpdir(), "turbo-ts-query-files-"));
    try {
      await prepareFixture(directory);
      const filePath = join(directory, "large.txt");
      const aliasPath = join(directory, "large-alias.txt");
      await writeFile(filePath, "x".repeat(1024 * 1024 + 1));
      if (process.platform !== "win32") await symlink("large.txt", aliasPath);

      const runQuery = async (
        query: string,
        metadataSize?: number,
        metadataKind?: "directory" | "file" | "symlink" | "other",
      ): Promise<{
        readonly exitCode: number;
        ...
        return { exitCode, output, rangeReads };
      };

      const preflight = await runQuery('{ file(path: "large.txt") { path } }');
      expect(preflight.exitCode).toBe(1);
      expect(preflight.output).toContain("file exceeds the 1 MiB safety limit");
      expect(preflight.rangeReads).toBe(0);

      const special = await runQuery(
        '{ file(path: "large.txt") { path } }',
        0,
        "other",
      );
      expect(special.exitCode).toBe(1);
      expect(special.output).toContain("file path is not a regular file");
      expect(special.rangeReads).toBe(0);

      const growthRace = await runQuery(
        '{ file(path: "large.txt") { path } }',
        1024 * 1024,
      );
      expect(growthRace.exitCode).toBe(1);
      expect(growthRace.output).toContain(
        "file exceeds the 1 MiB safety limit",
      );
      expect(growthRace.rangeReads).toBe(1);

      await writeFile(filePath, "x".repeat(1024 * 1024));
      const aliasTarget =
        process.platform === "win32" ? "large.txt" : "large-alias.txt";
      const deduplicated = await runQuery(
        `{ first: file(path: "large.txt") { path } second: file(path: "${aliasTarget}") { path } }`,
      );
      expect(deduplicated.exitCode).toBe(0);
      expect(deduplicated.rangeReads).toBe(1);

      const aliases = Array.from(
        { length: 9 },
        (_, index) => `file${index}: file(path: "large.txt") { path }`,
      ).join(" ");
      const aggregate = await runQuery(`{ ${aliases} }`);
      expect(aggregate.exitCode).toBe(1);
      expect(aggregate.output).toContain(
        "file query results exceed the 8 MiB safety limit",
      );
      expect(aggregate.rangeReads).toBe(1);
    } finally {
      await rm(directory, { force: true, recursive: true });
    }
  }, 30_000);
  ...
  it("resolves GraphQL dependencies from each ecosystem's owning lockfile", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-query-owning-lockfiles-"),
    );
    const cargoWorkspaceDirectory = join(directory, "rust");
    const cargoDirectory = join(cargoWorkspaceDirectory, "app");
    const uvDirectory = join(directory, "python/app");
    const cargoPackageId = `path+file://${cargoDirectory}#cargo-app@0.1.0`;
    try {
      await prepareFixture(directory);
      const configurationPath = join(directory, "turbo.json");
      const configuration = JSON.parse(
      ...
      const javascriptManifestPath = join(
        directory,
        "packages/app/package.json",
      );
      const javascriptManifest = JSON.parse(
        await readFile(javascriptManifestPath, "utf8"),
      ) as { dependencies: Record<string, string> };
      javascriptManifest.dependencies["js-external"] = "1.0.0";
      await writeFile(
        javascriptManifestPath,
        `${JSON.stringify(javascriptManifest, undefined, 2)}\n`,
      );
      const javascriptLockfile = join(directory, "pnpm-lock.yaml");
      await writeFile(
        javascriptLockfile,
        `lockfileVersion: '9.0'
importers:
...
`,
      ...
      const cargoLockfile = join(cargoWorkspaceDirectory, "Cargo.lock");
      await writeFile(
        cargoLockfile,
        `version = 4
...
`,
      ...
      await writeFile(
        ...
        '[project]\nname = "uv-app"\nversion = "0.1.0"\ndependencies = ["uv-external==3.0.0"]\n',
      );
      const uvLockfile = join(directory, "uv.lock");
      await writeFile(
        uvLockfile,
        `version = 1
...
name = "uv-app"
version = "0.1.0"
source = { editable = "python/app" }
dependencies = [{ name = "uv-external", version = "3.0.0" }]
...
`,
        ...
        packages: [
          {
            ...
            targets: [{ kind: ["lib"], name: "cargo_app" }],
          },
        ],
      });
      const lockfileReads = new Map<string, number>();
      let output = "";
      const exitCode = await Effect.runPromise(
        Effect.gen(function* () {
          const fileSystem = yield* FileSystemService;
          ...
          return yield* executeQuery({
            cwd: directory,
            query: "{ externalDependencies { items } }",
            schema: false,
            port: 8000,
          }).pipe(
            Effect.provide(
              Layer.mergeAll(
                Layer.succeed(FileSystemService, {
                  ...
                  readBytesRange: (path, offset, length) => {
                    if (
                      path === javascriptLockfile ||
                      path === cargoLockfile ||
                      path === uvLockfile
                    ) {
                      lockfileReads.set(
                        path,
                        (lockfileReads.get(path) ?? 0) + 1,
                      );
                    }
                    return fileSystem.readBytesRange(path, offset, length);
                  },
                ...
                Layer.succeed(ProcessService, {
                  ...
                  run: (request) => {
                    ...
                    if (request.command === "cargo") {
                      return Effect.succeed({
                        exitCode: 0,
                        stdout: "cargo 1.96.0-nightly\n",
                        stderr: "",
                        combinedOutput: "cargo 1.96.0-nightly\n",
                      });
                    }
                    if (request.command === "rustc") {
                      const identity =
      ...
      for (const lockfile of [javascriptLockfile, cargoLockfile, uvLockfile]) {
        expect(lockfileReads.get(lockfile)).toBe(1);
      }
    } finally {
      await rm(directory, { force: true, recursive: true });
    }
...
`,
  ...
  it("excludes transitive Cargo workspaces from query external dependencies", async () => {
    ...
    const packages = ["a", "b", "c"].map((name) => {
      const packageDirectory = join(cargoWorkspaceDirectory, name);
      return {
        id: `path+file://${packageDirectory}#rust-${name}@0.1.0`,
        name: `rust-${name}`,
        packageDirectory,
      };
    });
    try {
      ...
      await mkdir(cargoWorkspaceDirectory, { recursive: true });
      await writeFile(
        join(cargoWorkspaceDirectory, "package.json"),
        `${JSON.stringify({
          name: "synthetic-cargo-root",
          private: true,
          packageManager: "cargo@1.96.0",
        })}\n`,
      );
      await writeFile(
        join(cargoWorkspaceDirectory, "turbo.json"),
        `${JSON.stringify({
          futureFlags: { experimentalCargoWorkspaces: true },
          tasks: {},
        })}\n`,
      ...
      for (const package_ of packages) {
        await mkdir(join(package_.packageDirectory, "src"), {
          recursive: true,
        });
        await writeFile(
          join(package_.packageDirectory, "Cargo.toml"),
          `[package]\nname = "${package_.name}"\nversion = "0.1.0"\nedition = "2024"\n`,
        );
        await writeFile(join(package_.packageDirectory, "src/lib.rs"), "");
      }
      await writeFile(
        join(cargoWorkspaceDirectory, "Cargo.lock"),
        `version = 4
...
`,
      ...
      const cargoMetadata = JSON.stringify({
        ...
        target_directory: join(cargoWorkspaceDirectory, "target"),
        packages: packages.map(({ id, name, packageDirectory }) => ({
          id,
          name,
          version: "0.1.0",
          ...
          dependencies:
            name === "rust-a"
              ? [
                  {
                    name: "rust-b",
                    path: join(cargoWorkspaceDirectory, "b"),
                    source: null,
                  },
                ]
              : name === "rust-b"
                ? [
                    {
                      name: "rust-c",
                      path: join(cargoWorkspaceDirectory, "c"),
                      source: null,
                    },
                  ]
                : [
      ...
      const cargoCommand = join(commandDirectory, "cargo");
      const rustcCommand = join(commandDirectory, "rustc");
      await writeFile(
        cargoCommand,
        `#!/usr/bin/env node\nif (process.argv.includes("metadata")) process.stdout.write(${JSON.stringify(cargoMetadata)}); else console.log("cargo 1.96.0-nightly");\n`,
      );
      await writeFile(
        rustcCommand,
...
`,
  ...
  it("copies selected nested workspaces once and omits unselected ones", async () => {
    ...
    try {
      await prepareFixture(directory);
      const parentDirectory = join(directory, "packages/parent");
      const nestedDirectory = join(parentDirectory, "nested");
      await mkdir(nestedDirectory, { recursive: true });
      await writeFile(
        join(parentDirectory, "package.json"),
        `${JSON.stringify({ name: "synthetic-parent", private: true })}\n`,
      );
      await writeFile(join(parentDirectory, "source.txt"), "parent\n");
      await writeFile(
        join(nestedDirectory, "package.json"),
      ...
      expect(
        await readFile(
          join(directory, "result/packages/parent/source.txt"),
          "utf8",
        ),
      ).toBe("parent\n");
      expect(
        await readFile(
          join(directory, "result/packages/parent/nested/package.json"),
          "utf8",
        ).catch(() => undefined),
      ).toBeUndefined();
      expect(
  ...
  it("rejects unsafe prune output aliases and escaping package symlinks", async () => {
    ...
    try {
      ...
      await symlink(".", join(directory, "source-alias"));
      await expect(
        execFilePromise(process.execPath, [
          candidate,
          "prune",
          "synthetic-app",
          "--out-dir=source-alias/packages",
          "--cwd",
          directory,
        ]),
      ).rejects.toThrow(/repository package or workspace control directory/);
      ...
      await rm(join(directory, "source-alias"));
      await mkdir(join(directory, "config"));
  ...
  it("assigns nested changes to the most specific workspace", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-nested-affected-"),
    );
    try {
      await prepareFixture(directory);
      await writeFile(
      ...
      const nestedDirectory = join(directory, "packages/app/nested");
      ...
      await writeFile(
        ...
        `${JSON.stringify({
          ...
          version: "1.0.0",
          private: true,
          scripts: { build: "node -e \"console.log('nested build')\"" },
        })}\n`,
      );
      const sourcePath = join(nestedDirectory, "source.txt");
      await writeFile(sourcePath, "changed\n");
      const changedPaths = new TextEncoder().encode(
        "packages/app/nested/source.txt\0",
      );
      let listOutput = "";
      let queryOutput = "";
      await Effect.runPromise(
      ...
      expect(JSON.parse(queryOutput)).toEqual({
        data: {
          ...
          affectedTasks: {
            items: [{ fullName: "synthetic-nested#build" }],
          },
        },
      });
    } finally {
  ...
  it("preserves leading dots in affected workspace owner paths", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-dot-workspace-affected-"),
    );
    try {
      await prepareFixture(directory);
      await rename(join(directory, "packages"), join(directory, ".packages"));
      await writeFile(
        join(directory, "pnpm-workspace.yaml"),
        "packages:\n  - .packages/*\n",
      );
  ...
  it("matches repository-global affected inputs case-insensitively on Windows", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "turbo-ts-global-input-case-"),
    );
    try {
      await prepareFixture(directory);
      const configurationPath = join(directory, "turbo.json");
      ...
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          expect(
            yield* executeQuery({
              cwd: directory,
              query: "{ affectedPackages { items { name } } }",
              schema: false,
              port: 8000,
            }).pipe(
              Effect.provide(
  ...
  it("traces dependency-task reasons through affected task edges", async () => {
    ...
    try {
      await prepareFixture(directory);
      const applicationConfigurationPath = join(
        directory,
        "packages/app/turbo.json",
      );
      ...
      libraryManifest.scripts.compile =
        "node -e \"console.log('library compile')\"";
      await writeFile(
        libraryManifestPath,
        `${JSON.stringify(libraryManifest, undefined, 2)}\n`,
      );
